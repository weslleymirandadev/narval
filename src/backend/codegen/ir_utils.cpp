...
llvm::Function* create_function(IRGenerationContext& context, const std::string& name, std::unique_ptr<Function>& function_type) {
    if (!function_type) return nullptr;
    std::vector<llvm::Type*> param_types;
    for (const auto& p : function_type->paramstype)
        param_types.push_back(context.nv_type_to_llvm(p));
    auto* ret_type = context.nv_type_to_llvm(function_type->returntype);
