...
void TryStatementNode::codegen(nv::IRGenerationContext& ctx) {
    ...
    if (catches.empty()) {
        ...
        for (size_t i = 0; i < catches.size(); ++i) {
            ...
            /* Bind da variável de exceção (se houver) */
            ...
            /* Limpar exceção antes de executar o corpo */
        ...
        /* Nenhum catch correspondeu → relançar */
        B.SetInsertPoint(unhandled_bb);
        B.CreateCall(rethrow_fn, {});
        B.CreateUnreachable();
    }

    /* ── 6. Bloco FINALLY (opcional) ── */
    if (finally_bb) {
