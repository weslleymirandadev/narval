...
void DecrementExprNode::codegen(nv::IRGenerationContext& ctx) {
    ...
    if (operand->kind == NodeType::AccessExpression) { auto* acc = static_cast<AccessExprNode*>(operand.get());
        ...
        // Preparar self (Value*)
        auto* selfAlloca = ctx.create_alloca(ValueTy, "dec.self");
        b.CreateStore(base, selfAlloca);

        // Obter valor atual usando array_get_index_v
        auto* oldvAlloca = ctx.create_alloca(ValueTy, "dec.oldv");
        ...
        auto* tmp = ctx.create_alloca(ValueTy, "dec.tmp");
        ...
        auto* nextBox = ctx.create_alloca(ValueTy, "dec.next.box");
