...
/* derive the public key from a fixed 32-byte seed (RFC 8032 style).
   Used by af69d --key: seed -> pk, then sk[64] = seed || pk. */
int crypto_sign_seed_to_pk(u8 *pk, const u8 *seed)
{
  u8 d[64];
  gf p[4];
