...
// The runtime objects travel inside this binary (generated by CMake, see
// src/backend/runtime/CMakeLists.txt); linking a program must not need them on disk.
...
int run_batch_mode(const std::string& filename, bool build_only = false,
    ...
    try {
        ...
        // Prefer the embedded copy: write it to a temp file and link against that.
        ...
        const std::string blob_rt = no_std
            ? write_blob("nostd", narval_runtime_nostd_obj, narval_runtime_nostd_obj_len)
            : write_blob("std", narval_runtime_obj, narval_runtime_obj_len);
