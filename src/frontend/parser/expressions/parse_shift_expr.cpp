...
// Shifts bind tighter than the comparison operators and looser than the
// unary ones (`1 << 2 + 3` is `1 << (2 + 3)`), the C order.
std::unique_ptr<Node> parse_shift_expr(Parser* parser) {
    ...
    while (
        parser->current_token().type == TokenType::LEFT_SHIFT ||
        parser->current_token().type == TokenType::RIGHT_SHIFT
    ) {
