#include "frontend/parser/expressions/parse_primary_chain.hpp"
...
std::unique_ptr<Node> parse_primary_chain(Parser* parser, std::unique_ptr<Node> initial) {
    std::unique_ptr<Node> expr;

    if (initial) {
        expr = std::move(initial);
    } else {
    ...
    while (
        ...
        if (parser->current_token().type == TokenType::DOT) {
            ...
            std::unique_ptr<PositionData> pos_property = std::make_unique<PositionData>(
                line_property, column_property[0], column_property[1],
                position_property[0], position_property[1]
            );
